...
async def main():
    ...
    print(
        ...
        f'(e.g. https://t.me/{Fore.LIGHTYELLOW_EX}your_channel'
    ...
    async for message in client.iter_messages(channel_name, limit=None):
                ...
                if isinstance(entity, MessageEntityTextUrl):
                    ...
                    if 'https://t.me/' in url:
                        if match := re.match(
                            ...
                            print(
                                f"URL - {Fore.CYAN}https://t.me/"
                                f"{match[1]}{Style.RESET_ALL}"
                            )
        ...
        if message.text and isinstance(message.text, str):
            ...
            for match in matches:
                ...
                total_urls_scraped += 1  # Increment total count
                print(
                    f"URL - {Fore.CYAN}https://t.me/{match}{Style.RESET_ALL}"
                )
    ...
    print(
        f'URLs scraped: {total_urls_scraped} | New URLs added to'
        f'{channel_name}.csv: {new_urls_count} | {channel_name}'
        f'URL count: len({all_urls})'
    )
    ...
    print(
        f'Total URLs updated successfully. Saved to: {total_urls_filename}'
        f'| New URLs added: {new_total_urls_count} | Total: {len(total_urls)}'
    )
