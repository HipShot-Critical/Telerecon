...
async def get_user_information(client, identifier, username):
    try:
        ...
        print(f"{Fore.CYAN}Username:{Style.RESET_ALL} {user.username or 'N/A'}")  # noqa: E501
        print(f"{Fore.CYAN}First Name:{Style.RESET_ALL} {user.first_name or 'N/A'}")  # noqa: E501
        print(f"{Fore.CYAN}Last Name:{Style.RESET_ALL} {user.last_name or 'N/A'}")  # noqa: E501
        ...
        if user.photo:
            ...
            with open(
                os.path.join(user_directory, f'{username}_profile.jpg'), 'wb'
            ) as file:
                file.write(profile_photo)

            print(
                f"{Fore.CYAN}Profile Picture:{Style.RESET_ALL}"
                f"Downloaded and saved as {username}_profile.jpg"
            )
        ...
        if user.status:
            if isinstance(user.status, UserStatusOffline):
                ...
                print(
                    f"{Fore.CYAN}Last Seen:{Style.RESET_ALL}"
                    f"User was last seen at {last_seen_formatted}"
                )
        ...
        if hasattr(user, 'mutual_chats_count'):
            ...
            if common_chats is not None:
                print(
                    f"{Fore.CYAN}Common Chats:{Style.RESET_ALL} {common_chats}"
                )

        # Save user details to a text file in the user-specific directory
        with open(
            os.path.join(user_directory, f'{username}_userdetails.txt'), 'w'
        ) as details_file:
            ...
            if user.status:
                if isinstance(user.status, UserStatusOffline):
                    last_seen = user.status.was_online
                    last_seen_formatted = format_timestamp(
                        last_seen.timestamp()
                    )
                    details_file.write(
                        "Last Seen: User was last seen at"
                        f"{last_seen_formatted}\n"
                        )

                else:
                    details_file.write("Online Status: Online\n")

            else:
                details_file.write("Online Status: Offline\n")

            if hasattr(user, 'mutual_chats_count'):
                details_file.write(f"Common Chats: {common_chats}\n")

    except Exception as e:
        print(f"{Fore.RED}Error:{e}{Style.RESET_ALL} {str(e)}")
