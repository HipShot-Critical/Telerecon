...
async def scrape_user_messages(
    ...
    media_directory = os.path.join(
        user_directory, f"{sanitized_target_user.lstrip('@')}_media"
    ...
    async with TelegramClient(phone, api_id, api_hash) as client:
        try:
            ...
            async for post in client.iter_messages(
                ...
                try:
                    if isinstance(post.sender, User):
                        username = (
                            post.sender.username if post.sender.username else "N/A"  # noqa: E501
                        )
                        first_name = (
                            post.sender.first_name if post.sender.first_name else "N/A"  # noqa: E501
                        )
                        last_name = (
                            post.sender.last_name if post.sender.last_name else "N/A"  # noqa: E501
                        )
                    ...
                    print(
                        f"{Fore.RED}Error – retrieving sender"
                        f"information: {e}{Style.RESET_ALL}"
                    )
                    username = "N/A"
                    first_name = "N/A"
                    last_name = "N/A"
                ...
                # Check if the message has media (image or voice message)
                # and download_media is True
                ...
                if post.reply_to_msg_id:
                    ...
                    try:
                        ...
                        if isinstance(original_message.sender, User):
                            ...
                            sender_first_name = (
                                original_message.sender.first_name
                                if original_message.sender.first_name
                                else ""
                            )
                            sender_last_name = (
                                original_message.sender.last_name
                                if original_message.sender.last_name
                                else ""
                            )
                    ...
                    # Use the sender's username as the receiver's in a reply
            ...
            # Return both content and network data
...
async def main():
    target_user = input(
        f"\n{Fore.CYAN}Please enter the target"
        f"user's @username: {Style.RESET_ALL}"
    )
    target_channel = input(
        f"{Fore.CYAN}Please enter the target channel's"
        f"@username or invite link: {Style.RESET_ALL}"
    )
    download_media_option = input(
        f"{Fore.CYAN}Would you like to download media"
        f"from the channel (y/n)? {Style.RESET_ALL}"
    )
    ...
    sanitized_target_user = (
        target_user.replace("@", "").replace("_", "").replace("-", "")
    )
    ...
    channel_content, network_data = await scrape_user_messages(
        target_channel,
        target_user,
        user_directory,
        download_media,
    ...
    if message_count:
        ...
        csv_filename = os.path.join(
            user_directory, f"{sanitized_target_user}_messages.csv"
        )
        try:
            ...
            print(
                ...
                f"while saving to CSV: {e}{Style.RESET_ALL}"
            )  # Red text for errors
    else:
    ...
    network_csv_filename = os.path.join(
        user_directory, f"{sanitized_target_user.lstrip('@')}_network.csv"
    )
    try:
        network_df.to_csv(network_csv_filename, index=False)
        print(
            f"Successfully saved network interaction"
            f"data to {network_csv_filename}."
        )
    except Exception as e:
        print(
            f"{Fore.RED}An error occurred while saving"
            f"network data to CSV: {e}{Style.RESET_ALL}"
        )
